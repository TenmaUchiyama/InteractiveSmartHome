...
export const deleteNodeApi = async (req: Request, res: Response) => {
  try {
    const nodeId = req.params.id;
    console.log('Deleting node:', nodeId);
    await MongoDB.getInstance().deleteNode(nodeId);
    return res.status(200).send('Node deleted');
  } catch (error) {
    console.error(error);
    return res.status(500).send('Failed to delete node: ' + error.message);
  }
};
