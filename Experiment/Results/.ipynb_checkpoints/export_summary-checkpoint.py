...
def run_bradley_terry(pref_csv: str) -> str:
    ...
    edges = []
    for rnk in rankings:
        edges += [(idx[w], idx[l]) for (w,l) in pairs_from_ranking(rnk)]
    ...
    probs = ex / ex.sum()
    beta   = {COND_ORDER[i]: float(theta[i]) for i in range(4)}
    scores = {COND_ORDER[i]: float(probs[i]) for i in range(4)}
    ...
    def _ll(th):
        ...
        return s
    ll = _ll(theta); k_param = len(COND_ORDER)-1
    AIC = -2*ll + 2*k_param
