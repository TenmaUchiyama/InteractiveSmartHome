...
def sr_agent_node(state: State):
    ...
    elapsed_time = callback.system_end()

    print("==========[SR AGENT NODE]=========")
    print("OUTPUT: ", output["reasoning"])
    print("RESPONSE: ", output["response"])
    print(f"[SR AGENT TIME ELAPSED] {elapsed_time:.4f} sec")
    ...
    state.spatialAgent.metrics = {
        ...
        "system_time_elapsed": elapsed_time
    }
...
def pointing_spatial_node(state: PointingState):
    ...
    res = spatial_agent.invoke(state.input_prompt)
    ...
    print(f"[POINTING SPATIAL TIME ELAPSED] {elapsed_time:.4f} sec")
    return state
